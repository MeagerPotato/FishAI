 ...
 * form at the same dose, which is what makes the two an ordered pair for the fit; (5) the credit
 * never promotes an uncertain ask above a certain hit, and no priced knob can ungate it;
...
describe('the closing ask', () => {
  ...
  it('the belief form is pointwise at least the certain form at the same dose', () => {
    let compared = 0
    let strictlyMore = 0
  ...
  it('never promotes an uncertain ask above a certain hit, and no priced knob ungates it', () => {
    // The gate is `gated` alone: a certain hit is available and this candidate is not certain. The
    // credit is non-negative and is paid only to candidates the gate lets through, so a certain
    // hit - which already carries `certaintyBonus` - can never be overtaken by an uncertain ask.
    const ungatedPriced = withStyle({ closing: DOSE, exposure: 0, exposureCertain: true })
    ...
    for (const seed of ['closing-gate-a', 'closing-gate-b', 'closing-gate-c']) {
      play(seed, ON, (_s, view, n) => {
        ...
        // `exposureCertain` with no exposure appetite leaves the priced terms dead, so it may not
        // move a decision the closing credit is making: the two knobs are independent by design.
        expect(canonicalAction(decide(view, ungatedPriced, n))).toBe(canonicalAction(a))
        certainDecisions++
      })
    }
