...
import { chooseStyle } from '../lib/engine/bots/adaptive.ts'
...
const ADAPTIVE_ARM = ARM.startsWith('adaptive')
const ADAPTIVE_SPEC = Object.freeze({ adaptive: true })

/** The policy handed to `decide` for each seat. */
function policiesFor(g) {
  if (ADAPTIVE_ARM) return [0, 1, 2, 3, 4, 5].map(() => ADAPTIVE_SPEC)
  return stylesFor(g)
}

/**
 * The StyleParams actually in force for the acting seat — for an adaptive seat this is the style
 * `decide` itself resolves to, re-derived through the same `chooseStyle` call, so the score
 * rebuild and the ablations price the vector that really played.
 */
function effectiveStyle(view, pol) {
  if (pol !== ADAPTIVE_SPEC) return pol
  const s = STYLE_ROSTER[chooseStyle(view, ADAPTIVE_SPEC).style] ?? STYLE_ROSTER.balanced
  return ARM.endsWith('-nodefuse') ? strip(s) : s
}
...
for (let g = 0; g < GAMES; g++) {
  ...
  while (st.phase !== 'finished' && steps < 4000) {
    ...
    const style = effectiveStyle(view, pols[seat])
    const action = decide(view, pols[seat], steps)
    ...
    if (action.type === 'ask') {
      ...
      if (before.phase === 'playing' && before.hands[action.target].length > 0) {
        ...
        const preAct = decide(preView, pols[action.target], steps)
