 ...
 *
 *     node scripts/athena/bench-forward.mjs [--sizes S,M,L] [--games 12] [--reps 20]
 *
 ...
 * - **as the stub plays**: `decideStub` at every decision of every seat, with a per-seat incremental cache (the
...
console.log(`  host: ${os.cpus()[0].model}, ${os.cpus().length} logical CPUs, Node ${process.version}, one thread`)
console.log(`  load before: ${fmtLoad(loadBefore)}`)

/* ------------------------------------------------------------------------------------------- the sizes --- */

function timeSize(size) {
  const arch = A.ARCHS[size]
  const tInit = now()
  ...
  // as the stub plays / the network at every decision
  ...
  for (const g of games) {
    ...
    for (const s of g.states) {
      ...
      let t0 = now()
      const d = A.decideStub(net, v, cacheA[seat])
      let dt = now() - t0
      gameA += dt
      played.all.push(dt)
...
for (const size of SIZES) {
  ...
  console.log(`    as the stub plays: ${kinds}; a network decision ${mean(p.net).toFixed(3)} ms mean (p50 ${q(p.net, 0.5).toFixed(3)}, p95 ${q(p.net, 0.95).toFixed(3)}, max ${Math.max(...p.net).toFixed(1)}), any decision ${mean(p.all).toFixed(3)} ms mean; a six-seat game ${mean(p.game).toFixed(0)} ms mean (max ${Math.max(...p.game).toFixed(0)})`)
...
console.log('\n| size | actor weights | fold us/event | trunk+heads ms | stub: ms per network decision | stub: ms per six-seat game | every decision: ms | every decision: ms per six-seat game |')
console.log('|---|---|---|---|---|---|---|---|')
