 ...
 * `--only` skips the rest, and then updates the manifest's entries for exactly the papers it built,
 * keeping every other entry as it stands, so adding one paper does not rebuild eleven PDFs.
 */
...
// print sizes for some papers and nothing for the rest, with no way to tell which. So it updates
// the entries it built and keeps every other entry as the last run wrote it.
const MANIFEST = join(ROOT, 'src', 'pages', 'papers-manifest.json')
const previous = only !== null && existsSync(MANIFEST) ? JSON.parse(await readFile(MANIFEST, 'utf8')) : null
const merged = { ...(previous?.papers ?? {}), ...built }
const manifest = {
  note: 'Generated by scripts/build-papers.mjs. Do not edit by hand; run `npm run papers:build`.',
  generatedAt: new Date().toISOString(),
  engine: engineVersion,
  papers: Object.fromEntries(Object.keys(merged).sort().map((slug) => [slug, merged[slug]])),
}
await writeFile(MANIFEST, `${JSON.stringify(manifest, null, 2)}\n`, 'utf8')
console.log(
  only === null
    ? '\nsrc/pages/papers-manifest.json rewritten.'
    : `\nsrc/pages/papers-manifest.json: ${Object.keys(built).join(', ')} updated, the rest kept.`,
)
