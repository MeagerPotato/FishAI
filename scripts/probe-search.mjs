 ...
 * it play (what the guard is holding back); and the true hit rates of pick and played ask. MONET.md
 * 3.8aa adds the split by whether the pick was a certain hit (its p in the fast policy's ranking 1) -
...
let asks = 0, searched = 0, played = 0, hitPick = 0, hitPlayed = 0, bestHeld = 0, searchedCertain = 0, playedCertain = 0
const ownAdv = [], ownSe = [], trueAdv = [], trueBest = [], trueHeld = [], trueCertain = [], trueOpen = []
const t0 = Date.now()
  ...
  while (s.phase !== 'finished' && n++ < 5000) {
    ...
    if (!view.declareWindow && view.phase === 'playing') {
      ...
      if (d.info.searched) {
        ...
        // was the pick a certain hit in the fast policy's own ranking? (p 1; the top five carry p)
        const rp = (ENG.decideExplained(view, pol, seed).trace.ranked ?? []).find((r) => r.target === pick.target && r.card === pick.card)
        const certain = rp !== undefined && rp.p === 1
        if (certain) searchedCertain++
        if (d.info.played === 'candidate') {
          played++
          ownAdv.push(d.info.advantage)
          ownSe.push(d.info.se)
          const tv = rollTrue(a) - vPick
          trueAdv.push(tv)
          if (certain) { playedCertain++; trueCertain.push(tv) } else trueOpen.push(tv)
...
console.log(`=== probe-search: ${VERSION}${OVER ? ' ' + JSON.stringify(OVER) : ''} search ${JSON.stringify(params)}, ${GAMES} mirror games (${LABEL}-*), ${secs.toFixed(1)}s ===`)
...
console.log(`pick certain on ${searchedCertain} of searched; played there ${playedCertain} with true advantage ${fmt(stat(trueCertain))}; pick uncertain on ${searched - searchedCertain}; played there ${played - playedCertain} with true advantage ${fmt(stat(trueOpen))}`)
